...
class CoopList extends db
{
    ...
    {
        $sql = "select *  from Co_op_List  where co_list_id = '".$id."'";
        mysqli_query($this->db_link, "SET NAMES UTF8");
        if($result = mysqli_query($this->db_link, $sql)){
            $this->Close();
            return $result; 
    ...
    public function getAllDataLists()
    {
        $sql = "SELECT 
                    m_id, m_userID,  m_Firstname, m_Lastname, m_major, m_level, co_name
                FROM
